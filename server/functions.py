...
def get_predicted_value(patient_symptoms):
    ...
    disease = svc.predict([input_vector])[0]
    return diseases_list[disease]


# Function to get the disease description
def get_description(disease):
    desc = description[description['Disease'] == disease]['Description']
    return " ".join(desc.values) if not desc.empty else "No description available."
...
def cure_patient(disease):
    ...
    if not instruc.empty:
        instruc = instruc.tolist()  # Convert to list
    else:
        instruc = []
